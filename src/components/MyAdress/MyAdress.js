...
function MyAdress({ isAdresses, onPostUserAdress, onDeleteAdress, onChangeAdress }) {
    ...
    useEffect(() => {
        ...
        if (isAdresses && isAdresses.length) {
            setAddresses(isAdresses.slice(0, 3));
        }
    }, [currentUser, setValues, isAdresses]);
